...
**174 listings live in production, 1 hidden** (a pre-existing admin test row, "yoyo" — unchanged), confirmed directly via a fresh read-only query on 2026-09-12, after Discovery Workbench Batch 8's 33-listing import (see below) — which itself followed Batch 7's 27-listing import the same day (114 → 141 → 174). Before Batch 7's import, production had already reached 114 (up from the 106 recorded on 2026-09-11) — an unattributed +8, same pattern as the earlier +17: not produced by anything in this pass, most likely organic real-site submissions, not a full accounting. Re-verify before treating any of these numbers as current.
