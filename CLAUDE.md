...
**106 listings live in production, 0 hidden**, confirmed directly via a fresh read-only query on 2026-09-11 (up from the 89 recorded on 2026-09-05). The +17 delta was not investigated or attributed to any specific source — it wasn't produced by anything in this pass (no import/unhide ran between those two counts), and organic real-site submissions are the most likely explanation, but this is a count check, not a full accounting. Re-verify before treating either number as current.
...
## Discovery Workbench Batch 7 — current batch, staged in production, NOT yet researched

- Pushed locally with `workbench-sync.mjs --push --batch-size=100` (100 candidates, 0 photos — none of these place_ids have local photos on disk yet) and transferred to production the same way Batches 3/4/5 were.
- Confirmed live in production: `discovery_batch_rows` holds exactly batch_id `7`, 100 rows, 100 distinct place_ids, 100% match against the local set, 0 duplicates. Production listings unaffected throughout (106, 0 hidden, confirmed before and after).
- Local status as of 2026-09-11: `Remaining eligible (blank, not yet pushed/completed): 3017`.
- **Not yet researched by the intern.** Do NOT pull, purge, import, or publish Batch 7 until the intern's research is complete — as of now it is staged and visible to the intern, nothing more. Learn from Batch 5 above: check its actual review progress directly before assuming it's done, regardless of how much time has passed.
